...
#define ADCSR_SET       (0x4007u)  /* ADCSR setting value                    */
...
static uint32_t adc_read_single (void)
{
    ...
    else
    {
        ...
        adc_data_a7 = (int32_t)(((uint32_t)ADC.ADDRH >> ADC_BIT_SHIFT_6) & ADC_10BIT_SET);
    }
...
static void int_adc_end (uint32_t int_sense)
{
	...
	g_adc_result = ((uint32_t)ADC.ADDRH >> 4u);
