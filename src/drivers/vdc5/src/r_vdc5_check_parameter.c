...
* @brief        VDC5 driver checking arguments functions
******************************************************************************/
...
/* Valid range for HS */
#define VDC5_CPARA_HS_MAX          (16u)
/* Valid range for VS + VW */
#define VDC5_CPARA_VS_VW_MAX       (2039u)
/* Valid range for VS */
#define VDC5_CPARA_VS_MAX          (4u)
/* CLUT num */
...
{
    vdc5_error_t                    error;
    const vdc5_sclingdown_rot_t   * scldw_rt_tmp;
    ...
    scldw_rt_tmp = &param->sclingdown_rot;
    ...
    const vdc5_read_t   * const param)
{
    ...
    {
        /* Vertical width of the image read from frame buffer */
        ...
        /* Horizontal width of the image read from frame buffer */
...
vdc5_error_t VDC5_ImageColorMatrixCheckPrm (const vdc5_channel_t ch, const vdc5_color_matrix_t * const param)
{
    ...
    /* Offset (DC) adjustment (YG, B, and R) */
    ...
    /* Signal gain adjustment (GG, GB, GR, BG, BB, BR, RG, RB, and RR) */
    ...
    const vdc5_period_rect_t    * const enh_area)
{
    ...
    if (sharp_param != NULL)
    {
        /* Horizontal sharpness (H1, H2, and H3) */
    ...
    const vdc5_clut_t   * const param)
{
    ...
    /* The number of CLUT data values */
    ...
    /* CLUT data buffer address where CLUT data is to be stored (ARGB8888 format) */
    if (param->clut == NULL)
    {
        error = VDC5_ERR_PARAM_NULL;
        goto END;
...
static vdc5_error_t CheckRect (const vdc5_period_rect_t * const rect)
{
    ...
    if ((uint32_t)rect->vs < (uint32_t)VDC5_CPARA_VS_MAX)
    {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
    }
    ...
    if ((uint32_t)rect->hs < (uint32_t)VDC5_CPARA_HS_MAX)
    {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
    }
