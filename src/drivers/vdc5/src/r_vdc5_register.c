...
* @brief        VDC5 driver controlling VDC5 registers
******************************************************************************/
...
static void SetScalingDown(
    const vdc5_sclingdown_rot_t * const scldw_rot,
    ...
    *syscnt_panel_clk_reg   = syscnt_panel_clk_tmp;
    ...
    if (lvds_tmp != NULL)
    {
        ...
        *(regaddr_lvds->lpllsetr) &= (uint32_t)~VDC5_REG_BIT0;
        ...
        for (counter = 0; counter < (uint32_t)VDC5_LVDS_PLL_LOCK_RETRY_CNT; counter++)
        {   /* Delay while waiting for PLL to settle. */
            if ((*(regaddr_lvds->lpllmonr) & (uint32_t)VDC5_REG_BIT0) != 0u)
            {   /* Detected PLL lock */
                break;
            }
        }
    ...
    const vdc5_write_t  * const param)
{
    ...
    const vdc5_sclingdown_rot_t   * scldw_rot;

    scaler      = &vdc5_regaddr_scaler[ch][scaling_id];
    scldw_rot   = &param->sclingdown_rot;
    ...
    const vdc5_read_t   * const param)
{
    ...
    *(img_synthesizer->gr_flm6) |= (uint32_t)param->gr_format << VDC5_REG_SHIFT_28;
    /* The width of the horizontal valid period */
    ...
    *(img_synthesizer->gr_ab7) &= (uint32_t)~VDC5_REG_BIT0;
    ...
    if (cascade != VDC5_OFF)
    {
        ...
        {   /* Graphics 0 is used and graphics 1 is not used. */
    ...
    const vdc5_read_chg_t   * const param)
{
    ...
    if (param->gr_base != NULL)
    {
        *(img_synthesizer->gr_flm2) = (uint32_t)param->gr_base & (uint32_t)~VDC5_REG_MASK_0X00000007;
        ...
        if (gr_flm_sel_tmp == VDC5_GR_FLM_SEL_FLM_NUM)
        {
    ...
    const vdc5_noise_reduction_t    * const param)
{
    ...
    if (param != NULL)
    {
        /* Noise reduction parameter of Y/G signal */
        nr_param = &param->y;
        *(input_ctrl->imgcnt_nr_cnt0) &= (uint32_t)~VDC5_REG_MASK_0X00007F33;
        /* Maximum value of signal coring (absolute value) */
        ...
        *(input_ctrl->imgcnt_nr_cnt0) |= (uint32_t)nr_param->nr1d_gain;
        ...
        /* Noise reduction parameter of CB/B signal */
        nr_param = &param->cb;
        /* Maximum value of signal coring (absolute value) */
        ...
        *(input_ctrl->imgcnt_nr_cnt1) |= (uint32_t)nr_param->nr1d_gain << VDC5_REG_SHIFT_16;
        ...
        /* Noise reduction parameter of CR/R signal */
        nr_param = &param->cr;
        /* Maximum value of signal coring (absolute value) */
    ...
    const vdc5_alpha_blending_rect_t    * const param)
{
    ...
    if (param != NULL)
    {
        ...
        /* Rectangular area setting for alpha blending */
...
******************************************************************************/
/**************************************************************************//**
 * @brief       Setting external input video signal
 * @param[in]   ext_sig_tmp                 : 
 * @param[out]  input_ctrl              : Register address
 * @retval      None
******************************************************************************/
static void SetVideoExternalInput (
    const vdc5_ext_in_sig_t         * const ext_sig_tmp,
...
/**************************************************************************//**
 * @brief       Set background color for graphics
 * @param[in]   ch                  : Channel
 * @retval      None
******************************************************************************/
static void SetGrapicsBackgroundColor (const vdc5_channel_t ch)
...
/**************************************************************************//**
 * @brief       Set background color for scaler 0, scaler 1, graphics 0, and graphics 1
 *
 *              Description:<br>
 *              This function should be called only when graphics 0 or graphics 1 is selected.
 * @param[in]   ch                  : Channel
 * @param[in]   graphics_id
 * @param[in]   color_space         : GBR or YCbCr
 * @retval      None
******************************************************************************/
static void SetScalerBackgroundColor (
    const vdc5_channel_t        ch,
...
/**************************************************************************//**
 * @brief       Set LCD TCON - STVA/VS
 * @param[in]   lcd_tcon                : 
 * @param[out]  output_ctrl             : Register address
 * @retval      None
******************************************************************************/
static void SetLcdTcon_STVA_VS (
    const vdc5_lcd_tcon_timing_t        * const lcd_tcon,
...
/**************************************************************************//**
 * @brief       Set scaling-up-Down
 * @param[in]   w_read_fb               : 
 * @param[in]   gr_grc                  : 
 * @param[in]   adj_sel                 : 
 * @param[in]   ip_conversion           : IP conversion on/off
 * @param[in]   res_ds_wr_md            :
 * @param[out]  scaler                  : Register address
 * @retval      None
******************************************************************************/
static void SetScalingUpDown (
    const vdc5_width_read_fb_t  * const w_read_fb,
    const vdc5_period_rect_t    * const gr_grc,
    const vdc5_onoff_t                  adj_sel,
    const vdc5_onoff_t                  ip_conversion,
...
printf("Set-Scaling-Up-Down.........\n");

    hw_before_scld = 720*2;
    vw_before_scld = 240;

    hw_after_scld = 480;  
    vw_after_scld = 360;

printf(" SetScaling-Up-Down, before HV,after HV = %d,%d,%d,%d,\n", hw_before_scld ,vw_before_scld,hw_after_scld, vw_after_scld);
...
/*VVVVVVVVVVVVVVVVVVV*/
    ...
    *(scaler->scl0_us1) &= (uint32_t)~VDC5_REG_BIT4;
    ...
    {																						
    		...
    		*(scaler->scl0_ds1) |= (uint32_t)VDC5_REG_BIT4;											// Vertial Scale Down ON
    		*(scaler->scl0_us1) &= (uint32_t)~VDC5_REG_BIT4;										// Vertial Scale up OFF

        scaling_ratio = CalcScalingRatio(vw_before_scld, vw_after_scld, VDC5_ON, VDC5_OFF);
// 			printf(" SetScalingUpDown, V ratio %d,\n", scaling_ratio);
        *(scaler->scl0_ds6) = scaling_ratio;
    }
    else
    {
    	#if 0
    		*(scaler->scl0_ds1) &= (uint32_t)~VDC5_REG_BIT4;										// Vertial Scale Down OFF
    		*(scaler->scl0_us1) |= (uint32_t)VDC5_REG_BIT4;											// Vertial Scale up ON
        scaling_ratio = CalcScalingRatio(vw_before_scld, vw_after_scld, VDC5_ON, VDC5_OFF);
      #else  
...
printf(" SetScalingDown, H ratio %d,\n", scaling_ratio);

        *(scaler->scl0_ds4) &= (uint32_t)~VDC5_REG_MASK_0X0000FFFF;
        *(scaler->scl0_ds4) |= (scaling_ratio );
        
printf(" scaler->scl0_ds4), readback %x,\n", *(scaler->scl0_ds4));        
    }
    else
    {   /* OFF */
//        *(scaler->scl0_ds1) &= (uint32_t)~VDC5_REG_BIT0;
    ...
    const vdc5_regaddr_scaler_t * const scaler)
{
    ...
    printf("SetScalingUp before %d, after %d\n",w_before_scl,w_after_scl);
...
/**************************************************************************//**
 * @brief       Confirm whether the graphics enlargement will occur or not
 * @param[in]   graphics_id             : 
 * @param[in]   gr_flm_sel              : 
 * @param[in]   w_read_fb               : 
 * @param[in]   gr_grc                  : 
    ...
    const vdc5_onoff_t                  gr_enlarge)
{
    ...
    {   /* Normal graphics display */
        ...
        *(scaler->scl0_update) |= (uint32_t)(VDC5_REG_BIT13 | VDC5_REG_BIT4);
        ...
        if (graphics_id == VDC5_GR_TYPE_GR0)
        {
            VDC5_ShrdPrmSetGr0Scaler(ch, VDC5_OFF);
        }
    }
    else
    {   /* Video image or graphics enlargement */
        ...
        else
        {
            ...
            *(scaler->scl0_us3) |= (uint32_t)gr_grc->hw;
            ...
            printf(" SetScalerGraphics------vs,vw, hs,hw = %d,%d,%d,%d \n",gr_grc->vs,gr_grc->vw, gr_grc->hs, gr_grc->hw);
   ...
   #if 1
    				/* Scaling-up */
            SetScalingUp(w_read_fb, gr_grc, adj_sel_tmp, ip_conversion, res_ds_wr_md_tmp, scaler);
   #else
            printf("Call......SetScalingUpDown Set Scaling Up and Down....\n");
#if 0
            *(scaler->scl0_us2)  = (uint32_t)29 << VDC5_REG_SHIFT_16;									//V start position , for max capture range
            *(scaler->scl0_us2) |= (uint32_t)480;																			//V width						
            *(scaler->scl0_us3)  = (uint32_t)120 << VDC5_REG_SHIFT_16;								//H start
            *(scaler->scl0_us3) |= (uint32_t)800;																			//H width
#endif
//						SetScalingUp(w_read_fb, gr_grc, adj_sel_tmp, ip_conversion, res_ds_wr_md_tmp, scaler);
                      
 						SetScalingUpDown(w_read_fb, gr_grc, adj_sel_tmp, ip_conversion, res_ds_wr_md_tmp, scaler);
   #endif         
        ...
        *(scaler->scl0_update) |= (uint32_t)(VDC5_REG_BIT13 | VDC5_REG_BIT8 | VDC5_REG_BIT4 | VDC5_REG_BIT0);
        ...
        if (graphics_id == VDC5_GR_TYPE_GR0)
        {
            VDC5_ShrdPrmSetGr0Scaler(ch, VDC5_ON);
        }
    }
    ...
    const vdc5_onoff_t          gr_enlarge)
{
    ...
    gr0_scaler = VDC5_ShrdPrmGetGr0Scaler(ch);
    if ((cascade == VDC5_OFF) && (gr0_scaler != VDC5_OFF))
    {   /* Cascade connection OFF and graphics 0 is enabled */
    ...
    const vdc5_onoff_t          gr_enlarge)
{
    ...
    /* Display area for VIN */
    gr0_scaler = VDC5_ShrdPrmGetGr0Scaler(ch);
    if (gr0_scaler != VDC5_OFF)
    {   /* Graphics 0 is enabled */
        img_synthesizer_vin = &vdc5_regaddr_img_synthesizer[ch][VDC5_GR_TYPE_VIN];
        if (cascade == VDC5_OFF)
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer writing for scaler 0
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer writing for scaler 1
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer writing for OIR
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer reading for graphics 0
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer reading for graphics 1
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer reading for graphics 2
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer reading for graphics 3
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Activate/deactivate frame buffer reading for OIR
 * @param[in]   ch              : Channel
 * @param[in]   ability         : 
 * @retval      None
******************************************************************************/
...
/**************************************************************************//**
 * @brief       Set register update control register to update the frame buffer
...
/**************************************************************************//**
 * @brief       Set graphics display mode
 * @param[in]   ch                      : 
 * @param[in]   layer_id                : 
 * @param[in]   gr_disp_sel             : 
 * @retval      None
******************************************************************************/
static void SetGraphicsDisplayMode (
    const vdc5_channel_t                ch,
 ...
 * @param[in]   offset                  : 
 * @param[in]   gain                    : 
 * @param[out]  color_matrix            : Register address
 * @retval      None
******************************************************************************/
static void SetColorMatrix (
    const vdc5_colormtx_mode_t                  mtx_mode,
    ...
    const vdc5_regaddr_color_matrix_t   * const color_matrix)
{
    ...
    *(color_matrix->mtx_mode) = (uint32_t)mtx_mode;
    ...
    /* Offset (DC) adjustment and signal gain adjustment */
 ...
 * @param[in]   shp_h_on                : 
 * @param[in]   sharp_param             : 
 * @param[out]  img_qlty_imp            : Register address
 * @retval      None
******************************************************************************/
static void SetImageEnhancementSharpness (
    const vdc5_onoff_t                          shp_h_on,
...
/**************************************************************************//**
 * @brief       Set alpha blending area of a rectangle
 * @param[in]   pd_disp_rect        : rectangular area for alpha blending
 * @param[out]  img_synthesizer     : Register address
 * @retval      None
******************************************************************************/
static void SetAlphaRectArea (
    const vdc5_pd_disp_rect_t               * const pd_disp_rect,
...
/**************************************************************************//**
 * @brief       Convert the color format from gr_format format into 24-bit RGB888 format
 * @param[in]   gr_format           : Color format
 * @param[in]   input_color         : 
 * @retval      uint32_t
******************************************************************************/
static uint32_t ColorConversion (const vdc5_gr_format_t gr_format, const uint32_t input_color)
{
...
/**************************************************************************//**
 * @brief       Convert the color data from 4 bits value into 8 bits value
 * @param[in]   color_value         : 
 * @retval      uint32_t
******************************************************************************/
static uint32_t ColorConversionFrom4to8 (const uint32_t color_value)
{
...
/**************************************************************************//**
 * @brief       Convert the color data from 5 bits value into 8 bits value
 * @param[in]   color_value         : 
 * @retval      uint32_t
******************************************************************************/
static uint32_t ColorConversionFrom5to8 (const uint32_t color_value)
...
/**************************************************************************//**
 * @brief       Convert the color data from 6 bits value into 8 bits value
 * @param[in]   color_value         : 
 * @retval      uint32_t
******************************************************************************/
static uint32_t ColorConversionFrom6to8 (const uint32_t color_value)
...
/**************************************************************************//**
 * @brief       Convert the color data from ARGB8888 format into AGBR8888 format
 * @param[in]   color_value         : 
 * @retval      uint32_t
******************************************************************************/
static uint32_t ColorConversionIntoAgbr (const uint32_t color_value)
...
/**************************************************************************//**
 * @brief       Set color lookup table (CLUT)
...
/**************************************************************************//**
 * @brief       Set start threshold for gamma correction
 * @param[in]   gam_th          : Start threshold of area 1 to 31
 * @param[out]  gam_area        : Register address list
 * @retval      None
******************************************************************************/
static void Set_StartThreshold_Gamma (const uint8_t * gam_th, volatile uint32_t * const * const gam_area)
{
...
/**************************************************************************//**
 * @brief       Set gain adjustment for gamma correction
 * @param[in]   gam_gain        : Gain adjustment of area 0 to 31
 * @param[out]  gam_lut         : Register address list
 * @retval      None
******************************************************************************/
static void Set_GainAdjustment_Gamma (const uint16_t * gam_gain, volatile uint32_t * const * const gam_lut)
{
